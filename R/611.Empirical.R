  ...
  if ((class(sU) != "integer") & (class(sU) != "numeric") || length(sU) >1 || sU<=sL ) stop("'sU' has to be a number greater than sL")
   ...
   # library(TeachingDemos)				#To get HPDs
    ...
    for(i in 1:k)
    {
      ...
      LEBAQ[i]=stats::qbeta(alp/2,x[i]+av[i],n-x[i]+bv[i])
      UEBAQ[i]=stats::qbeta(1-(alp/2),x[i]+av[i],n-x[i]+bv[i])

      LEBAH[i]=TeachingDemos::hpd(stats::qbeta,shape1=x[i]+av[i],shape2=n-x[i]+bv[i],conf=1-alp)[1]
      UEBAH[i]=TeachingDemos::hpd(stats::qbeta,shape1=x[i]+av[i],shape2=n-x[i]+bv[i],conf=1-alp)[2]
    }
