...
#' @param sL - Lower support for MLE stats::optimization
#' @param sU - Upper support for MLE stats::optimization
...
empericalBAx<-function(x,n,alp,sL,sU)
{
  ...
  if ((class(sU) != "integer") & (class(sU) != "numeric") || length(sU) >1 || sU<=sL ) stop("'sU' has to be a number greater than sL")
    ...
    mle=stats::optim(c(0.1,0.1), likelhd, NULL,method = "L-BFGS-B",lower = sL, upper = sU, hessian = TRUE)$par
  ...
  #  library(TeachingDemos)				#To get HPDs
    ...
    LEBAQ=stats::qbeta(alp/2,x+av,n-x+bv)
    UEBAQ=stats::qbeta(1-(alp/2),x+av,n-x+bv)

    LEBAH=TeachingDemos::hpd(stats::qbeta,shape1=x+av,shape2=n-x+bv,conf=1-alp)[1]
    UEBAH=TeachingDemos::hpd(stats::qbeta,shape1=x+av,shape2=n-x+bv,conf=1-alp)[2]
